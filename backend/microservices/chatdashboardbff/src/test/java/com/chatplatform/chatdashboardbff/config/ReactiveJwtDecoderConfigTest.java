 ...
 */
class ReactiveJwtDecoderConfigTest {

  private final ReactiveJwtDecoderConfig config = new ReactiveJwtDecoderConfig();
  ...
  void buildsDecoderWithoutFetchingKeys() {
    assertThat(config.reactiveJwtDecoder(properties, config.audienceValidator(properties)))
        .isNotNull();
  }
}
